...
import gettext
import comun
from show_note_dialog import ShowNoteDialog
...
class ShowNotesDialog(Gtk.Dialog):
	def __init__(self,user):
		title = comun.APPNAME + ' | '+_('Show Notes')
		...
		self.treeview.append_column(Gtk.TreeViewColumn('Text', Gtk.CellRendererText(), text=1))		
		...
		self.button3.set_image(Gtk.Image.new_from_stock(Gtk.STOCK_REMOVE,Gtk.IconSize.BUTTON))
		self.button3.connect('clicked',self.on_button_remove_clicked)
	...
	def read_notes(self):
		for note in self.user.get_notes_sort(offset = self.offset):
				self.store.append([note['id'],note['text']])
